...
class TestSearch:
    ...
    def test_text_search(self):
        ...
        # First create a test store and add some data
        store_name = f"search_test_{int(time.time())}"
        self.test_stores.append(store_name)
        requests.post(f"{API_BASE}/vector-stores", json={"name": store_name})
        ...
        response = requests.post(f"{API_BASE}/search-by-id", json=search_data)
        assert response.status_code == 200
        result = response.json()
    ...
    def test_search_with_limit(self):
        ...
        store_name = f"search_limit_test_{int(time.time())}"
        self.test_stores.append(store_name)
        requests.post(f"{API_BASE}/vector-stores", json={"name": store_name})
